...
RandomAccessUpdate_LCG(u64Int TableSize) {
	...
	for (i=0; i<NUPDATE/BLOCK_SIZE; i++) {
		...
		for (j=0; j<BLOCK_SIZE; j++) {
				...
				tit_clear_0_7();
#endif
			table = tw_load_nc(uncached_addr, ran[j] >> (64 - logTableSize));
      			table ^= ran[j];
			tw_store_nc(uncached_addr, ran[j] >> (64 - logTableSize), table);
			//printf("ran[j] >> (64 - logTableSize) = %d\n", ran[j] >> (64 - logTableSize));
    		}
  	}
}
...
RandomAccess_LCG(unsigned long totalMemSize, int doIO, char *outFname, double *GUPs, int *failure) {
	...
	//ununcached_addr	= (int64_t*)(base_addr_nc + 0x10000);
...
#pragma omp parallel
	...
	for (i=0; i<TableSize; i++) {
		*(int64_t*)(uncached_addr + i) = 0;
	}
#endif
#if 1
	for (i=0; i<TableSize; i++) {
		tw_store_nc(uncached_addr, i, i);
		//*(int64_t*)(uncached_addr + i) = i;
	}
#if 0
	for (i=0; i<TableSize; i++) {
		if(i < 100)
			printf("tw_load_nc(uncached_addr, i) = %d, i = %d\n", tw_load_nc(uncached_addr, i), i);
	}
#endif
#endif
...
#endif
		//printf("i = %d\n", i);
		table = tw_load_nc(uncached_addr, temp >> (64 - (int)logTableSize));
    		table ^= temp;
		tw_store_nc(uncached_addr, temp >> (64 - (int)logTableSize), table);
  	}
	...
	printf("line:213\n");
  	for (i=0; i<TableSize; i++)
			...
			tit_clear_0_7();
#endif
    		if (tw_load_nc(uncached_addr, i) != i)
		{
      			temp++;
